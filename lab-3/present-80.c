...
void generate_key_schedule(const u80 key, uint64_t key_schedule[ROUNDS]) {
   ...
   for (int round = 0; round < ROUNDS; round++) {
      ...
      key_schedule[round] = (modified_key.b[0] << 56)
         | ((uint64_t)modified_key.b[1] << 48)
         | ((uint64_t)modified_key.b[2] << 40)
         | ((uint64_t)modified_key.b[3] << 32)
         | ((uint64_t)modified_key.b[4] << 24)
         | ((uint64_t)modified_key.b[5] << 16)
         | ((uint64_t)modified_key.b[6] << 8)
         | ((uint64_t)modified_key.b[7]);
